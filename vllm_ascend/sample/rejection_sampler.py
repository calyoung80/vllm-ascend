...
from vllm_ascend.sample.sampler import (_SAMPLING_EPS,
...
def strict_rejection_sample_tensor(
    ...
    token_offsets = torch.arange(
        max_spec_len, dtype=torch.int64, device=device)
    flat_indices = (
        cu_start.unsqueeze(1) + token_offsets.unsqueeze(0)).clamp(
    ...
    for draft_idx in range(max_spec_len):
        ...
        sampled_columns.append(torch.where(
            emit_target,
            target_column,
            placeholder_req,
        ))
        prefix_accepted = prefix_accepted & (
            ~has_draft | (draft_column == target_column))
    ...
    output_columns = torch.arange(
        max_spec_len + 1, dtype=torch.int64, device=device)
...
class AscendRejectionSampler(RejectionSampler):
    ...
    def forward(
        ...
        if draft_probs is None and target_indices is None:
            target_token_ids = _sample_target_token_ids_for_strict_verify(
                target_logits_tensor, sampling_metadata)
