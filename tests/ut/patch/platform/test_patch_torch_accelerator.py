...
def test_patch_torch_accelerator_redirects_memory_apis(monkeypatch):
    ...
    monkeypatch.setattr(torch.npu, "empty_cache", npu_empty_cache, raising=False)
    monkeypatch.setattr(torch.npu, "memory_stats", npu_memory_stats, raising=False)
    monkeypatch.setattr(torch.npu, "memory_reserved", npu_memory_reserved, raising=False)
    monkeypatch.setattr(torch.npu, "reset_peak_memory_stats", npu_reset_peak_memory_stats, raising=False)
